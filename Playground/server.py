...
from mira_controller import MiraController

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

arm: MiraController = None

@asynccontextmanager
async def lifespan(app: FastAPI):
    global arm
    print("[SERVER] Inicializando controlador Mira en COM6...")
    arm = MiraController(port="COM6", baudrate=115200)
    yield
    print("[SERVER] Cerrando puerto serial COM6...")
    if arm:
        arm.close()

app = FastAPI(title="Mira Interactive Dashboard Server", lifespan=lifespan)
...
async def get_index():
    ...
    return HTMLResponse("<h1>Dashboard de Mira inicializándose...</h1>")
...
GAMES_CATALOG = [
    {
        ...
        "icon": "♻️",
        "rel_path": "Playground/games/waste_sorting/main.py",
        "bundle_files": [
            {"source": "PID.py", "remote": "PID.py"},
            {"source": "WonderCam.py", "remote": "WonderCam.py"},
            {"source": "FanModule.py", "remote": "FanModule.py"},
            {"source": "Stepper.py", "remote": "Stepper.py"},
            {"source": "TM1640.py", "remote": "TM1640.py"},
    ...
    },
    {
        ...
        "icon": "🎯",
        "rel_path": "Playground/games/color_tracking_sorting/main.py",
        "bundle_files": [
            {"source": "PID.py", "remote": "PID.py"},
            {"source": "WonderCam.py", "remote": "WonderCam.py"},
            {"source": "FanModule.py", "remote": "FanModule.py"},
    ...
    },
    {
        ...
        "icon": "🎮",
        "rel_path": "Playground/games/handle_control/main.py",
        "bundle_files": [
    ...
    },
    {
        ...
        "icon": "📱",
        "rel_path": "Playground/games/multi_remote_control/main.py",
        "bundle_files": [
            {"source": "USBDevice.py", "remote": "USBDevice.py"},
            {"source": "RobotControl.py", "remote": "RobotControl.py"},
            {"source": "SuctionNozzle.py", "remote": "SuctionNozzle.py"},
...
async def run_game(game_id: str):
    ...
    try:
        if not arm or not arm.is_connected:
            return JSONResponse({"status": "error", "message": "El controlador Mira no está conectado"}, status_code=503)
        ...
        if game_id == "color_tracking_sorting":
            ...
            if "elif msg == PSB_R1 | PSB_PRESS:" in code_clean:
                target_line = [line_str for line_str in code_clean.splitlines() if "elif msg == PSB_R1 | PSB_PRESS:" in line_str][0]
                code_clean = code_clean.replace(target_line + "\n      nozzle.on()\n      which_button_press = msg", r1_patch)
        else:
