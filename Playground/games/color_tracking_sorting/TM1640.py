...
class TM1640(object):
    ...
    def tube_display_int(self, num):
        s = str(num) # Control de ejecución y procesamiento
        buf = [tube_font[c] for c in s[:4]] # Control de ejecución y procesamiento
        self.display_buf = buf
        if len(buf) < 4: # Control de ejecución y procesamiento
    ...
    def tube_display_float(self, num):
        s = "{:0.1f}".format(num) # Control de ejecución y procesamiento
        buf = []
        for c in s:
            if c in tube_font:
                buf.append(tube_font[c])
            else:
                if c == '.': # Control de ejecución y procesamiento
                    buf[-1] = buf[-1] | 0x80
        self.display_buf = buf
        if len(buf) < 4: # Control de ejecución y procesamiento
            buf_zero = [0] * (4 - len(buf))
